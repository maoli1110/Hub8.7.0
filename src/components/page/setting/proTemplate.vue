    ...
    <div class="proTemplate">
        ...
        <div v-show="!EditVisible">
            ...
            <el-table :data="tableData" style="width: 100%" :default-sort="{prop: 'date', order: 'descending'}"
                      class="proTemplate-table">
                <el-table-column label="序号" width="70" type="index" align='center'>
                </el-table-column>
                <el-table-column prop="projModelName" label="名称">
                </el-table-column>
                <el-table-column prop="updateUser" width="150" label="更新人">
                </el-table-column>
                <el-table-column prop="updateTime" width="150" label="更新时间" :formatter="formatTime">
                </el-table-column>
        ...
        <div class="proTemplate-edit" v-show="EditVisible">
                ...
                <el-col :span="10" style="padding:0 20px;">
                    ...
                    <div style="padding:11px 10px 10px 20px;border:1px solid #ddd;border-bottom:none;" class="clearfix">
                        <div class="handle-icon clearfix" style="float:left;">
                            <div @mouseover="showCreateNodes= true" class="clearfix"
                                ...
                                <div class="createNodes" v-show="showCreateNodes" @mouseout="showCreateNodes= false"
                                     style="z-index:2000;width:135px">
                        ...
                        <div class="sear-icon" style="position:relative">
                            ...
                            <el-input placeholder="请输入内容" class="" icon="search" :on-icon-click="searchProTree"
                                      v-model="tableSearchKey" style=''></el-input>

                            <div style="float:right;margin-top:6px">
                                <span id="expandBtn" class="icon-plus" title="展开"
                                      @click="expandNode('expand','proZtree')"></span>
                                <span id="collapseBtn " class="icon-cut" title="折叠"
                                      @click="expandNode('collapse','proZtree')"></span>
                            </div>
                            <el-icon class="el-icon-circle-close"
                                     style="vertial-align:middle;position:absolute;z-index:10;color:#ccc;right:100px;top:0px"
                                     v-show="tableSearchKey.length>0" @click.native="clearEvent"></el-icon>
                        </div>
                    </div>
            ...
            <el-dialog title="增加多个节点" :visible.sync="textAreaVisible" class="multi-textarea"
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button type="primary" id="proBtnOk" @click="textAreaVisible = false,addMoreNodes">确 定
                    </el-button>
                    <el-button @click="textAreaVisible = false">取 消</el-button>
                </div>
            </el-dialog>
            <!--表单编辑-->
            <el-dialog title="添加" :visible.sync="formEditVisible" style="width:0%;position:fixed;left:50%;opacity:0;"
        ...
        <div class="formEdit" v-show="formEditVisible">
            ...
            <div :form-info="formInfo2" class="form-edit-info">
                <div class="formParams">
                    <span
                        style="display:inline-block;width:217px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">单位工程：
                        <span style="color:#6595f2;font-weight:bold" :title="editTitle[0]">{{editTitle[0]}}</span>
                    </span>
                </div>
                <div class="formParams">
                    <span
                        style="display:inline-block;width:217px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">分部项目：
                        <span style="color:#6595f2;font-weight:bold" :title="editTitle[1]">{{editTitle[1]}}</span>
                    </span>
                </div>
                <div class="formParams">
                    <span
                        style="display:inline-block;width:217px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">分项工程：
            ...
            <div class="formEdit-body">
                ...
                <div class="ztree-struc" style="position:relative">
                    <template>
                        <el-select v-model="value_" placeholder="选择表名称"
                                   style="padding:10px 10px 0;width:88%;margin-left:1px;">
                            <el-option v-for="(item,index) in addFormOption" :key="item.value_" :label="item.label"
                                       :value="item.value_" @change="selectAdd">
                            </el-option>
                        </el-select>
                    </template>
                    <div style="position:relative;display:inline-block;">
                        <el-input id="searchMessage" icon="search" placeholder="请输入要搜索的内容"
                                  style="padding:10px 0px 10px 10px;width:80%" :on-icon-click="editSearchTree"
                                  v-model='ztreeSearch'></el-input>
                        <el-icon class="el-icon-circle-cross"
                                 style="position:absolute;z-index:3001;color:#ccc;right:12px;top:19px;height:44%;line-height:20px;font-size:14px;width:20px;background:#fff;"
                                 v-show="ztreeSearch.length>0" @click.native="clearEvent"></el-icon>
                    </div>
                        <div style="float:right;margin-top:16px">
                        <span id="expandBtn1" class="icon-plus" title="展开"
                              @click="expandNode('expand','editTree')"></span>
                        <span id="collapseBtn2 " class="icon-cut" title="折叠"
                              @click="expandNode('collapse','editTree')"></span>
                    </div>
                    ...
                    <template>
                        <el-select v-model="value" placeholder="请选择" style="padding:10px;width:88%">
                            <el-option v-for="(item,index) in options" :key="item.value" :label="item.label"
                                       :value="item.value" @change="selectTestAval">
                            </el-option>
                        </el-select>
                    </template>
                    <template>
                                    ...
                                    <ul class="form-table">
                                        ...
                                        <li v-for='(childitemName,index) in childitem.childs'>
                                            ...
                                            <div class="tabs-icon clearfix">
                                                <span class="icon-tabs-eyes"
                                                      @click="itemView(value_,childitemName.formId)" title="预览"></span>
                                                <span class="icon-tabs-del"
                                                      @click="itemDelete(item.typeName,childitem.typeName,childitemName,index)"
                                                      title="删除"></span>
                                            </div>
                                        </li>

                                    </ul>
                                    <!--<p class="form-total"  >已选表单{{childitem.childs.length}}</p>-->
                                </div>
...
<script>
    ...
    let repeat;
    //newCount =  sessionStorage.getItem('newCount');
    export default {
        data() {
            return {
                setting: {
                        ...
                        this.typeList.forEach((el, index1) => {
                            el.childs.forEach((el, index2) => {
                                if (elForm.typeId == el.typeId) {
                                    el.childs.push({
                                        typeId: elForm.typeId,
                                        modelId: elForm.modelId,
                                        formId: elForm.formId,
                                    ...
                                    el.childs.forEach((el, index2) => {
                                        ...
                                        if (child) {
                                            ...
                                            childs.push({
                                                typeId: el.typeId,
                                                modelId: this.value_,
                                                formId: treeNode.formId,
                                                formName: treeNode.formName
                                            });
...
//                this.EditVisible = false;
            ...
            edit() {
                ...
                if (treeNode.nodeName == '全部') {
                    this.$message('根节点不能进行重命名操作！', '提示', {
                        confirmButtonText: '确定',
                    });
                    return false;
                }
...
//                    var childNodes = parentNode.children;
            ...
            remove(e) {
                ...
                if (this.nodeId == '0001') {
                    this.$alert('根节点无法删除', '提示', {
                        confirmButtonText: '确定',
                    });
                    return;
                }
                this.$confirm('所选节点将被删除，是否确认?', '删除节点', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
...
//                return this.typeList.child.length ;
            }
        }*/
    }
</script>
