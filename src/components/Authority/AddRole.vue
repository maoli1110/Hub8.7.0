            ...
            <div style="border:1px solid #e6e6e6">
                <el-carousel trigger="click" height="195px" arrow="always" :interval="99999999">
                    <el-carousel-item v-for="(items,index) in clientInformation_" :key="index">
                        <div style="padding:0 20px;margin-top:40px">
                            <el-card :body-style="{ padding: '0px' }" v-for="(item, i) in items" @click.native="selectClient(item)" :key="i" class="card">
                                <div style="text-align:center;margin-top:20px">
                                    <img :src="item.src" class="image" style="width:55%;height:55%;margin:0 auto">
                ...
                <div style="padding:20px">
                    <div style="position:relative;">
                        <el-checkbox v-model="checkAlls" @change="handleCheckAlls" style="position:absolute;left:39px;top:22px;z-index:1000">全部
                        </el-checkbox>
                    </div>

                    <el-table ref="multipleTable" :data="authoritedTableData" border tooltip-effect="dark">
                        <el-table-column label="" width="150">
                            <template slot-scope="scope">
                                <el-checkbox v-model="scope.row.checkAll" @change="handleCheckAllChange($event,scope.row)">{{scope.row.name}}
                                </el-checkbox>
                            </template>
                        </el-table-column>
                        <el-table-column prop="address" label="权限列表" header-align='center' show-overflow-tooltip>
                            <template slot-scope="scope">
                                <div>
                                    <el-checkbox-group v-model="scope.row.checkedCities" style="padding:0 10px" @change="handleCheckedCitiesChange(scope.row.checkedCities,scope.row)">
                                        <el-checkbox class="el-checked-wrap" v-for="(city,i) in scope.row.cities" :title="city.authName" :label="city" :key="i">
                                            {{city.authName}}
                                        </el-checkbox>
                                    </el-checkbox-group>
                                </div>
            ...
            save() {
                ...
                types.updateRoleAuth(params).then(res => {
                    if (res.data.code == 200) {
                        this.$router.push({
                            path: `/authority/role-management`
                        });
                    }
