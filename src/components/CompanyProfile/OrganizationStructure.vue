...
export default {
    ...
    mounted() {
        ...
        this.initTreeNode = function() {
            $("#organization-tree").children().empty();
            getOrgTreeList().then(res => {
                ...
                for (let [key, value] of entries(tempzNodes)) {
                    ...
                    $.fn.zTree.init($("#ztree-" + key), self.setting, value);
                }
                $("#organization-tree").width(self.getOrgTreeWidth());
            });
        }
        this.initTreeNode();
    },
...
.el-popover .pop-header .enterprise-name,
.el-popover .pop-manager-list .name {
    font-size: 14px;
    font-weight: bold;
    padding: 20px 10px;
}
